...
String   fncHtml_buildPage()
{
	...
	szReply += "\n</style>";
	szReply += "</head><body>";
	szReply += "<input type = \"button\" value = \"Refresh\" onclick = \"javascript: window.location = '/';\" / >";
	...
	szReply += "\n</fieldset>";
	szReply += "\n<fieldset><legend>Actuators:</legend>";
	for (int i = 0;i < 4;i++)
	{
		szReply += fncHtmlRelay(i);
...
String fncHtmlRelay(unsigned int Irelay)

{
	...
	szCommands += "\n<p>Rele " + String(Irelay) + " <span class=\"" + szColor + "\">" + szStatus + "</span>";
...
void loop(void) {
	...
	g_Mod_Sns_DHT22.loop(g_interval_now);
	g_Mod_Act_Relay[0].loop(g_interval_now);
	g_Mod_Act_Relay[1].loop(g_interval_now);
	g_Mod_Act_Relay[2].loop(g_interval_now);
	g_Mod_Act_Relay[3].loop(g_interval_now);
}
